...
func cancel_all_orders() {
    for id, order:= range data.Orders {
        if (order.Open) {
            cancel_order(data.Venue, data.Stocks[0], id)
        }
    }
}
...
func execute_strategy (strategy string) {
    ...
    case "level5":
        {
            buyPrice := int(quoteHistory.lastTopAskPrice*4/3);
            sellPrice := int(quoteHistory.avgTopBidPrice*4/3);
            if  data.Positions[data.Stocks[0]].Owned >= -200 {
                //buyPrice = int(quoteHistory.minTopAskPrice)
            }
            ...
            lastAskOrder := data.Orders[quoteHistory.lastAskId]
            lastBidOrder := data.Orders[quoteHistory.lastBidId]
            if data.Positions[data.Stocks[0]].Owned < 5000 && !lastBidOrder.Open {
                ok, id, filled := place_order(data.Venue, data.Stocks[0], "buy", data.Id, buyQty, buyPrice, "limit")
                if (ok) {
                    fmt.Printf("Buy Order sent id:%d price %d filled:%d\n", id, buyPrice, filled)
                    quoteHistory.lastBidId = id;
...
func init_web_sockets() {
    ...
    urlTicker:= fmt.Sprintf("wss://api.stockfighter.io/ob/api/ws/%s/venues/%s/tickertape",data.Id,data.Venue);
    wsTicker, errTicker:= websocket.Dial(urlTicker, "", origin)
    globals.wsTicker= wsTicker
    if errTicker!= nil {
        log.Fatal(errTicker)
    }
}

func update_ticker_ws() {
    for ;; {
        var msg = make([]byte, 512)
        _ , errTicker:= globals.wsTicker.Read(msg)
        if errTicker!= nil {
            log.Fatal(errTicker)
        }
        //s := string(msg[:n])
